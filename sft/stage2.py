...
def process(data: List[Dict]):
    """stage2: data has been processed in stage1"""
    return data
...
def prepare_user_data(tokenizer, test_data: List[Dict]) -> List[Dict]:
    ...
    for idx, item in enumerate(test_data):
        ...
        # print(
        #     f"=====Length of evaluation prompt: {len(tokenizer.encode(formatted_input))}====="
        # )
...
def main():
    ...
    # ===== 数据读取与预处理 =====
    train_file = f"stage1/{args.dataset}_stage_1_{args.split}.jsonl"
    output_file = f"stage2/{args.dataset}_stage_2_{args.split}.jsonl"
    with open(output_file, "w") as f:
        pass

    start_id = 11000

    data = read_file(train_file)[start_id:]
    print(train_file)
    ...
    for batch_idx, user_batch in enumerate(user_batches, start=1):
        ...
        for u_idx, user in user_batch:
            targets = [t for t in user["stage3"] if math.exp(t["logprob1"]) > 0.9]
            if len(targets) > 5:
                targets = random.sample(targets, 5)
            if len(targets) <= 3:
                continue
            for target in targets:
                history = [his["item"] for his in user["stage2"]]
                prompt = build_preference_prompt(
                    tokenizer, history, target["item"], user["preference_1"]
                )
        ...
        with open(output_file, "a+", encoding="utf-8") as fout:
            for u_idx in passed_uid:
                ...
                final_user["reasoning_preference_list_2"] = reasoning_pref_by_uid[u_idx]
                final_user["preference_2"] = final_pref_by_uid[u_idx]
                final_user["thinking_2"] = final_think_by_uid[u_idx]
                final_user["selected_target_ids_2"] = selected_targets_by_uid[u_idx][0]
