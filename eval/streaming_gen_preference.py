...
def build_prompt(
    tokenizer: AutoTokenizer, history: List[Dict[str, str]], past: Optional[str]
) -> str:
...
def main():
    ...
    for idx, item in tqdm(enumerate(dataset), total=len(dataset)):
        ...
        if last:
            ...
            mid_position = len(item["history"]) // 2
        history1 = item["history"][:mid_position]
    ...
    for i, out in enumerate(outputs):
        ...
        data[i]["last_profile"] = parse_output(raw)
    ...
    for i, item in tqdm(enumerate(data), total=len(data)):
        if last:
            ...
            history2 = item["history"][
                mid_position : 2 * mid_position
            ]  # whole history contains target
        else:
            mid_position = len(item["history"]) // 2
            history2 = item["history"][mid_position:]

        past = item["last_profile"]
        prompt = build_prompt(tokenizer, history2, past)
        item["raw_prompt_next"] = prompt
        texts.append(prompt)

    outputs = llm.generate(texts, sampling_params=sampling)
    for i, out in enumerate(outputs):
        raw = out.outputs[0].text
        data[i]["second_response"] = raw
    ...
    output_file = (
        f"preferences/{model_name}/{data_source}_preference_streaming_{model_name}.json"
    )
